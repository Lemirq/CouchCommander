...
export function ConnectionStatus({ isConnected, serverIP, onConnect, onDisconnect }: ConnectionStatusProps) {
					...
					<div className="space-y-4">
						<div className="space-y-2">
							...
							<div className="flex gap-2">
								...
								/>
								<Button size="sm" onClick={handleConnect} disabled={isConnecting || !ipInput.trim()}>
									{isConnecting ? <Loader2 className="h-3 w-3 animate-spin" /> : 'Connect'}
								</Button>
							</div>
						</div>
						<p className="text-sm text-muted-foreground">Make sure your laptop and phone are on the same Wi-Fi network</p>
					</div>
				)}
			</CardContent>
		</Card>
